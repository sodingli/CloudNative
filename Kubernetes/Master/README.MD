...
kubernetes 要创建证书，用我们之前创建好的证书文件就可以了。
...
从 [`CHANGELOG`页面](https://github.com/kubernetes/kubernetes/blob/master/CHANGELOG.md) 分别下载 `client` 或 `server` tarball 文件
...
`/etc/kubernetes/token.csv` 这个文件不存在可以新建。
`touch /etc/kubernetes/token.csv`
...
KUBE_API_ADDRESS="--advertise-address=172.16.97.244 --bind-address=172.16.97.244 --insecure-bind-address=172.16.97.244"
#
...
KUBE_ETCD_SERVERS="--etcd-servers=https://127.0.0.1:2379,https://172.20.0.114:2379,https://172.20.0.115:2379"
#
...
**创建 kube-scheduler的serivce配置文zhi
这个也不要太多的改动，只要配置正确，就能正常启动。
